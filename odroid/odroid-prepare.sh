...
# ============================================================================
# Odroid Development Environment Setup Script
# ============================================================================
# Optimized version for Odroid devices with Ubuntu (ARM64 architecture)
...
# ============================================================================
# Odroid Specific Configuration
# ============================================================================

detect_odroid() {
    log_info "Detecting Odroid hardware..."
    ...
    # Check for Odroid-specific files
    if [ -f /proc/cpuinfo ]; then
        local hardware=$(grep -i "hardware" /proc/cpuinfo | head -n1 || echo "Unknown")
...
main() {
    ...
    echo -e "${BOLD}${CYAN}║          Odroid Development Environment Setup                              ║${RESET}"
    echo -e "${BOLD}${CYAN}╚════════════════════════════════════════════════════════════════════════════╝${RESET}"
    echo ""
    ...
    # Install eza (if available for ARM, otherwise use standard ls)
    if apt-cache show eza &>/dev/null; then
        log_info "Installing eza..."
        apt install -y eza
    else
        log_warning "eza not available for ARM, using standard ls"
    fi
    ...
    if [ "$current_user" == "root" ]; then
        current_user=$(logname 2>/dev/null || echo "odroid")
    fi
